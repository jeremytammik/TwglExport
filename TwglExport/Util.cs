...
namespace TwglExport
{
  class Util
  {
    const string _caption = "Tiny WebGL Exporter";
    ...
    /// </summary>
    public static double MaxCoord( XYZ a )
    {
        ...
        bool includeType )
    {
      ...
      if( true /* Command._toExportDictionary.ContainsKey( cat ) */ )
      {
        foreach( Parameter p in parameters )
        {
          key = p.Definition.Name;
          ...
          if( true /*Command._toExportDictionary[cat].Contains( key )*/ )
          {
            if( !a.ContainsKey( key ) )
            {
              if( StorageType.String == p.StorageType )
